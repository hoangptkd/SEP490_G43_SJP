...
function EmployerJobsPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const payload: any = {
      ...
      salaryMin: formData.salaryType === 'range' ? Number(formData.salaryMin) : undefined,
                        ...
                        <div className="flex-1 w-full lg:w-auto">
                          ...
                          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-gray-500 text-sm mb-4">
                            ...
                            <span className="flex items-center gap-1.5">
                              ...
                              <strong className="text-emerald-600 font-medium">{job.salaryType === 'negotiable' ? 'Thỏa thuận' : job.salaryType === 'fixed' ? `${(job.salaryMax || job.salaryMin)?.toLocaleString() || 0} VNĐ` : `${job.salaryMin ? job.salaryMin.toLocaleString() : 0} - ${job.salaryMax ? job.salaryMax.toLocaleString() : 0} VNĐ`}</strong>
                            </span>
                            <span className="hidden sm:inline text-gray-300">•</span>
                            <span className="flex items-center gap-1.5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6 mb-8 bg-gray-50 p-6 rounded-xl border border-gray-100">
              <div>
                ...
                <div className="font-semibold text-emerald-600">
                  {viewingJob.salaryType === 'negotiable' ? 'Thỏa thuận' : viewingJob.salaryType === 'fixed' ? `${(viewingJob.salaryMax || viewingJob.salaryMin)?.toLocaleString() || 0} VNĐ` : `${viewingJob.salaryMin?.toLocaleString() || 0} - ${viewingJob.salaryMax?.toLocaleString() || 0} VNĐ`}
                </div>
              </div>
              <div>
